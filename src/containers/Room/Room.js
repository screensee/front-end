...
class Room extends PureComponent {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    ...
    if (
      nextProps.room &&
      _.get(nextProps.room, 'id') !== _.get(prevState.room, 'id')
    ) {
      result = {
        room: nextProps.room
      };
  ...
  componentDidMount() {
    ...
    if (!this.props.room && this.props.id) {
      makeRequest('post')(createUrl.roomJoin(this.props.id))().then(room => {
        this.setState({
          room
        });
        makeRequest('get')(createUrl.messGet(room.id))().then(messages => {
          this.setState({
            messages
          });
        });
  ...
  onUserRoomAction = isJoin => user => {
    this.setState(state => {
      let newParticipants;
      if (isJoin) {
        if (!_.includes(state.room.participants, user)) {
          newParticipants = [...state.room.participants, user];
        }
      } else {
        const index = state.room.participants.indexOf(user);
        if (index !== -1) {
      ...
      return {
        room: {
          ...state.room,
          participants: newParticipants !== undefined ? newParticipants : state.room.participants,
        }
      };
    });
  };
  ...
  renderUserIcons = () => {
    ...
    if (this.state.room) {
      ...
      toRender = participants.map(elem => (
        <StyledUserIcon name={elem} key={elem} img="" />
      ));
    }
    return toRender;
  };
    ...
    return (
      <RoomWrapper>
        ...
        <SettingsBar
          ...
          defaultUrl={_.get(room, 'videoLink')}
        />
        {videoId !== '' && (
